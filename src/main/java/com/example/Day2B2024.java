...
public class Day2B2024 {
    public static void main(String[] args) {
        ...
        if (inputs.size() > 0) {
            ...
            ArrayList<ArrayList<Integer>> badInputs = new ArrayList<ArrayList<Integer>>();
            for (String i : inputs) {
                ...
                Integer secondNextLevel = splitDataList.get(2);
                Integer difference = currentLevel - nextLevel;
                Integer differenceSecond = currentLevel - secondNextLevel;
                String increasingOrDecreasing = "neither";
                if (difference < 0 && differenceSecond < 0) increasingOrDecreasing = "increasing";
                else if (nextLevel- secondNextLevel < 0) {
                    splitDataList.remove(0);
                    increasingOrDecreasing = "increasing";
                }
                else if (difference > 0 && differenceSecond > 0) increasingOrDecreasing = "decreasing";
                else if (nextLevel- secondNextLevel > 0) {
                    splitDataList.remove(0);
                ...
                for (int z = 0;z < splitDataList.size() - 1;z++) {
                    ...
                    try {
                        secondNextLevel = splitDataList.get(z + 2);
                    }
                    catch(Exception e) {
                        end = true;
                    }
                    if (increasingOrDecreasing == "neither") {
                        splitDataList.remove(z);
                        badInputs.add(splitDataList);
                        break;
                    }
                    if (increasingOrDecreasing == "increasing") {
                        difference = nextLevel - currentLevel;
                        if (!end) differenceSecond = secondNextLevel - currentLevel;
                    }
                    if (increasingOrDecreasing == "decreasing") {
                        difference = currentLevel - nextLevel;
                        if (!end) differenceSecond = currentLevel - secondNextLevel;
                    }
                    if (difference < 1 || difference > 3) {
                        if (!end && differenceSecond < 1 || !end && differenceSecond > 3) {
                           splitDataList.remove(z);
                        }
                        else splitDataList.remove(z + 1);
                        badInputs.add(splitDataList);
                        break;
                    }
                ...
                System.out.println(badInputs);
            }

            // Loop through each bad report and check to see if it is safe or not after it has been changed.
            for (int k = 0;k < badInputs.size();k++) {
                System.out.println(badInputs.get(k));
                ArrayList<Integer> currentBadInput = badInputs.get(k);

                // Check to see if the input is increasing or decreasing
                Integer currentLevel = currentBadInput.get(0);
                Integer nextLevel = currentBadInput.get(1);
                Integer difference = currentLevel - nextLevel;
                String increasingOrDecreasing = "neither";
                ...
                // Loop through levels and as soon as it is unsafe break out. If it makes it to the end then it is safe
                for (int z = 0;z < currentBadInput.size() - 1;z++) {
                    currentLevel = currentBadInput.get(z);
                    nextLevel = currentBadInput.get(z + 1);
                    ...
                    // If we make it through the loop, it's safe, add 1 to the number of safe inputs
                    if (z == currentBadInput.size() - 2) amountSafe += 1;
                }
